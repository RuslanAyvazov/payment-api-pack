create or replace package body client_data_api_pack is
    ...
    if p_client_data is not empty then
      ...
      for i in p_client_data.first .. p_client_data.last loop
        ...
        if (p_client_data(i).field_id is null) then
          raise_application_error(client_common_pack.c_error_code_invalid_input_parameter,
                                  client_common_pack.c_error_msg_empty_field_id);
        end if;
      
        if (p_client_data(i).field_value is null) then
          raise_application_error(client_common_pack.c_error_code_invalid_input_parameter,
                                  client_common_pack.c_error_msg_empty_field_value);
        end if;
      end loop;
    else
      raise_application_error(client_common_pack.c_error_code_invalid_input_parameter,
                              client_common_pack.c_error_msg_empty_collection);
    end if;
  ...
  begin
    if not g_is_api and not client_common_pack.is_manual_change_allowed() then
