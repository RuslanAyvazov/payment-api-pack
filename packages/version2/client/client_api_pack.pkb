create or replace package body client_api_pack is
  ...
  begin
    if p_client_id is null then
      raise_application_error(client_common_pack.c_error_code_invalid_input_parameter,
                              client_common_pack.c_error_msg_empty_object_id);
    end if;
  
    if p_reason is null then
      raise_application_error(client_common_pack.c_error_code_invalid_input_parameter,
                              client_common_pack.c_error_msg_empty_reason);
    end if;
  ...
  begin
    if not g_is_api
       and not client_common_pack.is_manual_change_allowed() then
      raise_application_error(client_common_pack.c_error_code_manual_changes,
                              client_common_pack.c_error_msg_manual_changes);
    end if;
  end;

  procedure check_client_delete_restriction is
  begin
    if not client_common_pack.is_manual_change_allowed() then
      raise_application_error(client_common_pack.c_error_code_delete_forbidden,
                              client_common_pack.c_error_msg_delete_forbidden);
    end if;
  end;
  ...
  begin
    ...
    if v_is_active = c_inactive then
      raise_application_error(client_common_pack.c_error_code_inactive_object, client_common_pack.c_error_msg_inactive_object);        
    end if;
    
  exception
    when no_data_found then -- такой клиент вообще не найден
      raise_application_error(client_common_pack.c_error_code_object_notfound, client_common_pack.c_error_msg_object_notfound);
    when client_common_pack.e_row_locked then -- объект не удалось заблокировать
      raise_application_error(client_common_pack.c_error_code_object_already_locked, client_common_pack.c_error_msg_object_already_locked);
  end;
